...
1. **啟動伺服器 (Windows 推薦)**：
   直接在資料夾中雙擊執行 `run.bat`，即可透過選單一鍵啟動 Web 介面。
   *(或者手動在終端機輸入 `python server.py`)*
2. **開啟瀏覽器**：
   執行 `run.bat` 會自動開啟；若手動啟動，請前往 `http://127.0.0.1:8000` 進入儀表板。
3. **介面功能**：
...
4. **停止伺服器**：
   您可以直接點擊網頁側邊欄下方的「Stop Server」按鈕來安全關閉伺服器，或者在運行伺服器的終端機視窗中按下 `Ctrl + C`。
